...
# Entries Frame
entries_frame = Frame(root, bg="#4682B4")
...
comboGender['values'] = ("Male", "Female","Prefer Not to Say")
...
lblDepartment = Label(entries_frame, text="Department", font=("Calibri", 16), bg="#535c68", fg="white")
lblDepartment.grid(row=4, column=0, padx=10, pady=10, sticky="w")

txtDepartment = Text(entries_frame, width=55, height=3, font=("Calibri", 16))
txtDepartment.grid(row=5, column=0, columnspan=4, padx=10, sticky="w")
...
def getData(event):
    ...
    gender.set(row[5])
    contact.set(row[6])
    txtDepartment.delete(1.0, END)
    txtDepartment.insert(END, row[7])
...
def add_employee():
    if txtName.get() == "" or txtAge.get() == "" or txtDoj.get() == "" or txtEmail.get() == "" or comboGender.get() == "" or txtContact.get() == "" or txtDepartment.get(
            1.0, END) == "":
        messagebox.showerror("Erorr in Input", "Please Fill All the Details")
        return
    db.insert(txtName.get(),txtAge.get(), txtDoj.get() , txtEmail.get() ,comboGender.get(), txtContact.get(), txtDepartment.get(
            1.0, END))
